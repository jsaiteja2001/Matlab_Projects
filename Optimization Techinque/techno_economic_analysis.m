...
p_npv=7.3*upv;%kW >>0.130w each pv,14 pv in string;%rated power at reference condition
...
x=1:1:24;
bar(averagedaysolar);figure(gcf);xlim([0 24]);ylabel('Power(kW)');title('average daily output power from pv');

figure
%mean solar power output during the day
[averagedaywind]=HourlytoDaily(Pw);
x=1:1:24;
bar(averagedaywind);figure(gcf);xlim([0 24]);xlabel('hours');ylabel('Power(kW)');title('average daily output power from wind turbine');
