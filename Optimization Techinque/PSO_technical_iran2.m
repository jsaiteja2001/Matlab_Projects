...
%hade bala
C=3;%cost
W=0.15;%lose of load probability%
K=0.99;%renewable energy factor%
nvars=1;%only one system
houses=15;
LB=[15 0 15 0 0]; % Lower bound of problem
UB=[45 3 15 10 4]; % Upper bound of problem
%%
max_it=20;%70%100;% Maximum number of iterations
NPOP=5;%30%20;% Number of population
...
%rnwfct_best=inf;
for i=1:NPOP
    ccc=4;%a value for cost
    ww=0.5;% a value for lose of load probability%
    kkk=2;%renewable energy factor%
    ff=0;
%C=0.9;%price of electricity
%W=0.2;%lose of load probability%
%K=0.99;%renewable energy factor%
while  ccc>=C |ww>=W % LOLP/REfactor
        particle(i).position(1,:)=unifrnd(15,45,1,nvars);%pv kW
        particle(i).position(2,:)=unifrnd(0,3,1,nvars);%autonomy days
    ...
    particle(i).lpsp=LPSP;
    particle(i).renewable_factor=renewable_factor;
    particle(i).best.position=particle(i).position;
    particle(i).best.dump=particle(i).dump;
    particle(i).best.lpsp=particle(i).lpsp;
    particle(i).best.renewable_factor=particle(i).renewable_factor;
    
    if particle(i).best.dump<globalbest.dump & particle(i).best.lpsp<globalbest.lpsp& particle(i).best.renewable_factor<globalbest.renewable_factor
...
for u=1:max_it
    vv=0;
    for i=1:NPOP
        price_electricity=4;%price of electricity
        LPSP=0.5;% a value for lose of load probability%
        renewable_factor=2;%
        bb=0;
%C=0.9;%price of electricity
%W=0.2;%lose of load probability%
...
%         kkk=2*100;%renewable energy factor%
        while price_electricity>=C|(LPSP)>=W 
            ...
            % plot&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&77
            grid on;
        subplot(2,2,1),plot(i,LPSP,'--rs','LineWidth',2,...
                'MarkerEdgeColor','k',...
                'MarkerFaceColor','g',...
                'MarkerSize',5);
            title('LPSP')
        ...
        hold on;
        grid on;
        subplot(2,2,2),plot(i,price_electricity,'--rs','LineWidth',2,...
                'MarkerEdgeColor','k',...
                'MarkerFaceColor','g',...
                'MarkerSize',5);
            title('COE')
        ...
        hold on;
        grid on;
        subplot(2,2,3),plot(i,renewable_factor,'--rs','LineWidth',2,...
                'MarkerEdgeColor','k',...
                'MarkerFaceColor','g',...
                'MarkerSize',5);
            title('renewable factor')
        drawnow
        hold on;
         grid on;
        subplot(2,2,4),plot(i,Edump,'--rs','LineWidth',2,...
                'MarkerEdgeColor','k',...
                'MarkerFaceColor','g',...
                'MarkerSize',5);
            title('Edump')
...
%         sol(i).pos(3,:)=round(particle(i).position(3,:));
    ...
    objective(u,:)=globalbest.position;
%     objective(u,2)=globalbest.lpsp;
%     objective(u,3)=globalbest.dump;
...
result=[p_npv,ad,houses,nwt,LPSP,price_electricity,renewable_factor,b(1),b(2),b(3),b(4),b(5)];
 subplot(2,2,1),plot(bi,LPSP,'--rs','LineWidth',2,...
                'MarkerEdgeColor','b',...
                'MarkerFaceColor','r',...
                'MarkerSize',5);
            
 subplot(2,2,2),plot(bi,price_electricity,'--rs','LineWidth',2,...
                'MarkerEdgeColor','b',...
                'MarkerFaceColor','r',...
                'MarkerSize',5);
subplot(2,2,3),plot(bi,renewable_factor,'--rs','LineWidth',2,...
                'MarkerEdgeColor','b',...
                'MarkerFaceColor','r',...
                'MarkerSize',5);
subplot(2,2,4),plot(bi,Edump,'--rs','LineWidth',2,...
                'MarkerEdgeColor','b',...
                'MarkerFaceColor','r',...
                'MarkerSize',5);            
%bar(result)
