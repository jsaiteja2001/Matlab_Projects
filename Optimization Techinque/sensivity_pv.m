...
%reliability
%sum(load-pv-wind+battery)/sum(load)
k=0;
for t=1:1:8640
...
% [price_electricity] = economic(diesel,Pl,Fg,cwh,p_npv);
[price_electricity] = economic_fast(diesel,Pl,Fg,cwh,p_npv,nwt);
k=p_npv*2;
prc(k,1)=price_electricity;
prc(k,2)=lpsp;
prc(k,3)=b(1);%pv contribution
prc(k,4)=b(2);%wind contribution
prc(k,5)=b(3);%battery contribution
prc(k,6)=b(4);%diesel contribution
end
ali=[Pp(1:168)',Pw(1:168)',Eb(1:168)',diesel(1:168)',Pl(1:168)'];
 figure,subplot(2,1,1),x=1:168;bar(x,ali(:,1:4),'group');colormap jet,subplot(2,1,2),x=1:168;bar(x,ali(:,5),'group')
