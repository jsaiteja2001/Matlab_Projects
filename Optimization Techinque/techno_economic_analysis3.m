%sensivity analysis on autonomy days
%% solar power
clear all
close all
clc
...
if Pw(t)+Pp(t)>=(Pl(t)/uinv)
   ...
   %^^^^^^^^^^^^^^DISCHARGE^^^^^^^^^^^^^^^^^^^
   [Eb,Edump,Edch,diesel,time1,diesel_power] = dicharge(Pw,Pp,Eb,Ebmax,uinv,Pl,t,Pg,Ebmin,Edump,Edch,Ech,diesel,time1,diesel_power);
    contribution(1,t)=Pp(t);contribution(2,t)=Pw(t);contribution(3,t)=Edch(t);contribution(4,t)=diesel_power(t);
end
...
for t=1:1:8640
    ...
    if Pl(t)>(Pp(t)+Pw(t)+Eb(t)+diesel(t))
        k=k+1;
    end
...
prc(ad*2,1)=price_electricity;
prc(ad*2,2)=reliability;
prc(ad*2,3)=b(1);%pv contribution
prc(ad*2,4)=b(2);%wind contribution
prc(ad*2,5)=b(3);%battery contribution
prc(ad*2,6)=b(4);%diesel contribution
end
