...
%% wind turbine
%#######inputs##############inputs####################inputs##
MCH=5;%choose the model>>model two
...
% [price_electricity] = economic(diesel,Pl,Fg,cwh);
 ...
 Edump=sum(Edump);
%  prc(ad-2,nwt+1,p_npv)=price_electricity;
%  rlb(ad-2,nwt+1,p_npv)=LPSP;
prc(ad,p_npv)=price_electricity;
 rlb(ad,p_npv)=LPSP;
 ref(ad,p_npv)=renewable_factor;
 j=j+1;
 result(:,j)=[p_npv;ad;houses;nwt;nPng;LPSP;price_electricity;renewable_factor;Edump];
...
% imagesc(prc);figure(gcf);
% imagesc(rlb);figure(gcf);
% 
% prc1=prc/max((max(prc)));
% rlb1=rlb/max((max(rlb)));
% xxxx=prc1+rlb1;